...
def main(args):
    ...
    for e in range(init_epoch, num_epoch):
        for j in range(init_step, batch_per_epoch):
            ...
            decoder_unpacked, decoder_hidden = decoder(decoder_inputs[:-1, :], encoder_hidden, encoder_unpacked,
                                                       encoder_lengths)
            ...
            if args.self_critical > 1e-5:
                ...
                for j in range(batch_size):
                    ...
                    reward = Variable(torch.Tensor([sample_score["rouge-1"]['f'] - greedy_score["rouge-1"]['f']]),
                                      requires_grad=False)
            ...
            if i % c['test_step'] == 0:
                ...
                for j in range(n_test):
                    ...
                    test_encoder_unpacked, test_encoder_hidden = encoder(test_encoder_inputs, test_encoder_lengths,
                                                                         return_packed=False)
                    # we don't remove the last symbol
                    test_decoder_unpacked, test_decoder_hidden = decoder(test_decoder_inputs[:-1, :],
                                                                         test_encoder_hidden, test_encoder_unpacked,
                                                                         test_encoder_lengths)
                    trg_len, batch_size, d = test_decoder_unpacked.size()
                    # remove first symbol <SOS>
                    test_ce_loss = CEL(test_decoder_unpacked.view(trg_len * batch_size, d),
                                       test_decoder_inputs[1:, :].view(-1))
                    test_loss += test_ce_loss.data

                    test_enc_input = (
                    test_encoder_inputs[:, 0].unsqueeze(1), torch.LongTensor([test_encoder_lengths[0]]))
        ...
        if history['valid_loss'][-1] < history['best_loss']:
            ...
            # early stopping
            logger.info(
                since(start) + "Early stopping at epoch {0}, best result at epoch {1}".format(e, history['best_epoch']))
            return
